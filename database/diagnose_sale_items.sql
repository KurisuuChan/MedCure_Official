-- Diagnostic script to check sale_items table structure and verify top products function
...
-- 3. Test current top products function
SELECT * FROM get_top_selling_products(30, 5);

-- 4. Manual verification of top selling calculation
SELECT 
    si.product_id,
    p.generic_name,
    p.brand_name,
    ...
    SUM(COALESCE(si.subtotal, 0)) as total_revenue_subtotal,
    COUNT(*) as sale_count
FROM sale_items si
